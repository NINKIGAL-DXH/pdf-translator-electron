...
class OpenAIlikedTranslator(OpenAITranslator):
    ...
    envs = {
        ...
        "OPENAILIKED_REASONING_EFFORT": "",  # e.g. "none", "low", "medium", "high"
    }
    CustomPrompt = True
    ...
    def __init__(
        ...
        self.stream = stream_val == "true"
        # Support reasoning_effort for thinking models (e.g. gemma, qwen)
        reasoning_effort = self.envs.get("OPENAILIKED_REASONING_EFFORT", "").strip()
        if reasoning_effort:
            self.options["extra_body"] = {"reasoning_effort": reasoning_effort}

    def do_translate(self, text) -> str:
        """Override to support configurable streaming and reasoning_effort."""
        # Support reasoning_effort for thinking models (check env at runtime)
        import os as _os
        extra_body = None
        reasoning_effort = _os.environ.get("OPENAILIKED_REASONING_EFFORT", "").strip()
        if reasoning_effort:
